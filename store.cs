...
namespace Myshop
{
    public partial class store : Form
    {
        ...
        private void dataGridViewshow()
        {
            ...
            MySqlConnection conn_ = new MySqlConnection("datasource=127.0.0.1;port=3306;username=root;password=;database=myshop;");
            DataSet ds = new DataSet();
            conn_.Open();

            MySqlCommand cmd_;
            cmd_ = conn_.CreateCommand();
            cmd_.CommandText = "SELECT * FROM `food`";
            ...
            cmd_ = conn_.CreateCommand();
            cmd_.CommandText = $"SELECT * FROM `salehis` WHERE username = '{register.Apiwat}' and status = 0";
        ...
        string conn = "datasource=127.0.0.1;port=3306;username=root;password=;database=myshop;";
        string sql;
        MySqlConnection con;
        MySqlCommand cmd;
        ...
        {
            DialogResult dialogResult = MessageBox.Show("คุณต้องการเพิ่มสินค้าเข้าตะกร้าหรือไม่", " ", MessageBoxButtons.YesNo);
            if (dialogResult == DialogResult.Yes)
            {
                ...
                { //select from stock มาละเช็คว่าเหลือสินค้าเท่าไร ละนำสินค้าที่มีมาลบกับสินค้าที่เราเลือก
                    ...
                    if (balances >= 0)
                    {   
                        sql = $"UPDATE `food` SET `amount` = '{balances}' WHERE id = '{id}'";
                        con = new MySqlConnection(conn);
                        cmd = new MySqlCommand(sql, con);
                        con.Open();
                        ...
                        sql = $"INSERT INTO `salehis` (`username`, `name`, `amount`, `price`, `date`, `status`) " +
                            $"VALUES ('{register.Apiwat}', '{nametextBox.Text}', " +
        ...
        private void repair_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                sql = $"DELETE FROM salehis WHERE id = '{id}'";
                con = new MySqlConnection(conn);
                cmd = new MySqlCommand(sql, con);
                con.Open();
                int rows1_ = cmd.ExecuteNonQuery();
        ...
        private void pay_Click(object sender, EventArgs e)
        {
            ...
            sql = $"UPDATE `salehis` SET `status` = '1' WHERE username = '{register.Apiwat}' and status = 0";
            con = new MySqlConnection(conn);
            cmd = new MySqlCommand(sql, con);
            con.Open();
            int rows1_ = cmd.ExecuteNonQuery();
        ...
        {
            e.Graphics.DrawString("Pizza Compliler", new Font("TH SarabunPSK", 20, FontStyle.Bold), Brushes.Black, new Point(268, 40));
            ...
            e.Graphics.DrawString("    เบอร์ติดต่อ 0930878790  ", new Font("TH SarabunPSK", 16, FontStyle.Bold), Brushes.Black, new Point(45, 110));
            e.Graphics.DrawString("     Promtpay 0930878790", new Font("TH SarabunPSK", 16, FontStyle.Bold), Brushes.Black, new Point(45, 140));
            ...
            string sql6 = $"SELECT * FROM `salehis` WHERE username = '{register.Apiwat}' and status = 0";
            MySqlConnection conn6 = new MySqlConnection("datasource=127.0.0.1;port=3306;username=root;password=;database=myshop;");
            ...
            e.Graphics.DrawString("รวมทั้งสิ้น    " + จำนวนน + "   ชิ้น", new Font("TH SarabunPSK", 16, FontStyle.Regular), Brushes.Black, new Point(312, y +45));
            ...
            e.Graphics.DrawImage(new Bitmap(@"D:\food\QRcode.png"), new Point(300 + -40, 180 + 560));
        }
